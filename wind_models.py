...
if generator_path is not None:
    ...
    plot_gan_training(history, title='Learning curve (GAN)', save_path='plots/GAN_LEARNING_CURVE_WIND.png')

results = evaluate_model(generator, test_dataloader, "GAN_WIND", save_path='evaluations/GAN_EVALUATION.pkl', device=device)
plot_wind_forecasts(generator, prediction_loader, save_path='plots/GAN_FORECASTS_WIND.jpg')

gan_crps = results['crps'] # (days, 24)
gan_energy = results['energy'] # (days,)
gan_vario = results['variogram'] # (days,)
gan_quantiles = results['quantile'] # (99, days, 24)
...
plot_vae_training(history, title='Learning curve (VAE)', save_path='plots/VAE_WIND.jpg')
plot_wind_forecasts(vae, prediction_loader, save_path='plots/VAE_FORECASTS_WIND.jpg')
plot_correlations(vae, c_pred, train_loader.x_scaler, save_path='plots/VAE_CORRELATIONS_WIND.jpg', title='VAE correlations (Wind)')

results = evaluate_model(vae, test_dataloader, "VAE_WIND", save_path='evaluations/VAE_EVALUATION.pkl', device=device)
vae_crps = results['crps'] # (days, 24)
vae_energy = results['energy'] # (days,)
vae_vario = results['variogram'] # (days,)
vae_quantiles = results['quantile'] # (99, days, 24)
...
plot_nf_training(history, title='Learning curve (NAF)', save_path='plots/NAF_LEARNING_CURVE_WIND.png')
plot_wind_forecasts(flow, prediction_loader, save_path='plots/NAF_FORECASTS_WIND.jpg')
plot_correlations(flow, c_pred, train_loader.x_scaler, save_path='plots/NAF_CORRELATIONS_WIND.jpg', title='NAF correlations (Wind)')

results = evaluate_model(flow, test_dataloader, "NAF_WIND", save_path='evaluations/NAF_EVALUATION.pkl', device=device)

naf_crps = results['crps'] # (days, 24)
naf_energy = results['energy'] # (days,)
naf_vario = results['variogram'] # (days,)
naf_quantiles = results['quantile'] # (99, days, 24)
...
plot_nf_training(history, title='Learning curve (NSF)', save_path='plots/NSF_LEARNING_CURVE_WIND.png')
plot_wind_forecasts(flow, prediction_loader, save_path='plots/NSF_FORECASTS_WIND.jpg')
plot_correlations(flow, c_pred, train_loader.x_scaler, save_path='plots/NSF_CORRELATIONS_WIND.jpg', title='NSF correlations (Wind)')

results = evaluate_model(flow, test_dataloader, "NSF_WIND", save_path='evaluations/NSF_EVALUATION.pkl', device=device)

nsf_crps = results['crps'] # (days, 24)
nsf_energy = results['energy'] # (days,)
nsf_vario = results['variogram'] # (days,)
nsf_quantiles = results['quantile'] # (99, days, 24)
...
plot_dm(losses, labels, h=1, save_path='plots/DM_TEST_WIND.png', title='Diebold-Mariano Test (wind)')
