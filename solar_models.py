...
from data.utils_data import GEFcomSolarLoader
...
dataset = pd.read_csv("data/solar_data.csv")
...
# ---------- Train GAN ----------
gan_study = optuna.load_study(study_name="solar", storage="sqlite:///tuning/gan_study.db")
...
discriminator, generator, gan_history = train_wgan_gp_(generator, discriminator, 
                                                       train_dataloader, validationloader=validation_dataloader, 
                                                       d_lr=D_LR, epochs=EPOCHS, gp_lambda=GP_LAMBDA, n_critic=N_CRITIC)
discriminator.save('models/GAN_DISCRIMINATOR_SOLAR.pth')
generator.save('models/GAN_GENERATOR_SOLAR.pth')
plot_gan_training(gan_history, title='Καμπύλη Εκπαίδευσης του WGAN-GP στην Ηλιακή Ενέργεια', 
                  save_path='plots/GAN_LEARNING_CURVE_SOLAR.png')

gan_results = evaluate_model(generator, test_dataloader, "GAN_SOLAR", 
                             save_path='evaluations/GAN_EVALUATION.pkl', device=device)
plot_solar_forecasts(generator, prediction_loader, 
                     save_path='plots/GAN_FORECASTS_SOLAR.jpg', filter_hours=filter_hours)
...
plot_correlations(generator, c_pred, train_loader.x_scaler, 
                  save_path='plots/GAN_CORRELATIONS_SOLAR.jpg', 
                  title='Συσχετίσεις του WGAN-GP στην Ηλιακή Ενέργεια')
...
vae, vae_history = train_vae_linear(vae, train_dataloader, 
                                    validation_dataloader, epochs=EPOCHS, 
                                    save_path='models/vae_SOLAR.pth', lr=LR)

plot_vae_training(vae_history, title='Καμπύλη Εκπαίδευσης του VAE στην Ηλιακή Ενέργεια', save_path='plots/VAE_SOLAR.jpg')
plot_solar_forecasts(vae, prediction_loader, save_path='plots/VAE_FORECASTS_SOLAR.jpg', filter_hours=filter_hours)
plot_correlations(vae, c_pred, train_loader.x_scaler, 
                  save_path='plots/VAE_CORRELATIONS_SOLAR.jpg', 
                  title='Συσχετίσεις του VAE στην Ηλιακή Ενέργεια')

vae_results = evaluate_model(vae, test_dataloader, "VAE_SOLAR", 
                             save_path='evaluations/VAE_EVALUATION.pkl', 
                             device=device)
...
naf_study = optuna.load_study(study_name="solar", storage="sqlite:///tuning/naf_study.db")
...
naf_history = naf.fit(train_dataloader, validation_dataloader=validation_dataloader,
                      epochs=EPOCHS, lr=LR, patience=20, device=device, save_path='models/NAF_MODEL_LOAD.pt')
plot_nf_training(naf_history, title='Καμπύλη Εκπαίδευσης του NAF στην Ηλιακή Ενέργεια', 
                 save_path='plots/NAF_LEARNING_CURVE_SOLAR.png')
plot_solar_forecasts(naf, prediction_loader, save_path='plots/NAF_FORECASTS_SOLAR.jpg', 
                     filter_hours=filter_hours)
plot_correlations(naf, c_pred, train_loader.x_scaler, 
                  save_path='plots/NAF_CORRELATIONS_SOLAR.jpg', 
                  title='Συσχετίσεις του NAF στην Ηλιακή Ενέργεια')

naf_results = evaluate_model(naf, test_dataloader, "NAF_SOLAR", 
                             save_path='evaluations/NAF_EVALUATION.pkl', 
                             device=device)
...
nsf_study = optuna.load_study(study_name="solar", storage="sqlite:///tuning/nsf_study.db")
...
nsf_history = nsf.fit(train_dataloader, 
                      validation_dataloader=validation_dataloader, 
                      epochs=EPOCHS, lr=LR, patience=20, 
                      device=device, 
                      save_path='models/NSF_MODEL_LOAD.pt')
plot_nf_training(nsf_history, 
                 title='Καμπύλη Εκπαίδευσης του NSF στην Ηλιακή Ενέργεια', 
                 save_path='plots/NSF_LEARNING_CURVE_SOLAR.png')
plot_solar_forecasts(nsf, prediction_loader, 
                     save_path='plots/NSF_FORECASTS_SOLAR.jpg', 
                     filter_hours=filter_hours)
plot_correlations(nsf, c_pred, train_loader.x_scaler, 
                  save_path='plots/NSF_CORRELATIONS_SOLAR.jpg', 
                  title='Συσχετίσεις του NSF στην Ηλιακή Ενέργεια')

nsf_results = evaluate_model(nsf, test_dataloader, "NSF_SOLAR", 
                             save_path='evaluations/NSF_EVALUATION.pkl', device=device)
...
plot_crps([gan_crps, vae_crps, naf_crps, nsf_crps], ['GAN', 'VAE', 'NAF', 'NSF'], 
          save_path='plots/CRPS_SOLAR.png', title='CRPS Μοντέλων στην Ηλιακή Ενέργεια')
plot_qs([gan_quantiles, vae_quantiles, naf_quantiles, nsf_quantiles], 
        ['GAN', 'VAE', 'NAF', 'NSF'], save_path='plots/QS_SOLAR.png', 
        title='QS Μοντέλων στην Ηλιακή Ενέργεια')

compare_models_on_solar_forecasts(prediction_loader, [generator, vae, naf, nsf], 
                                  save_path='plots/COMPARE_FORECASTS_SOLAR.png', 
                                  filter_hours=filter_hours)
...
plot_dm(losses, labels, h=x_dim-1, save_path='plots/DM_TEST_SOLAR.png', 
        title='Έλεγχος Diebold-Mariano στην Ηλιακή Ενέργεια')
...
plot_roc_many_scenarios(X_real, fake_by_model, n_runs=50,
                        title="Καμπύλς ROC του Ταξινομητή στην Ηλιακή Ενέργεια", 
                        save_path='plots/ROC_SOLAR.png')
